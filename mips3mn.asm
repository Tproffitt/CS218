...
msg1:		.asciiz "Surface Areas - Values:"
msg2:		.asciiz "\nSurface Areas - Stats: \n"
str1:		.asciiz	"   min  = "
str2:		.asciiz	"\n   max  = "
str3:		.asciiz	"\n   med  = "
...
shellSort:
	li $t0, 1					# h = 1   $t0 = h
firstWhile:						# while ( (h*3+1) < length) {
	mul $t0, $t0, 3			#	h = 3 * h + 1;
	add $t0, $t0, 1
	blt $t0, $a1, firstWhile	# }

secondWhile:					# while ( h>0 ) {
	move $t1, $t0						# $t1 = i
	sub $t1, $t1, 0						# i = h-1
outerFor:						#	for (i = h-1; i < length; i++) {
	bge $t1, $a1, outerForDone
	move $t5, $t1				#		tmp = lst[i];		//$t9 = tmp
	mul $t5, $t5, 4
	add $t5, $t5, $a0
	lw $t9, ($t5)
	move $t2, $t1				#		j = i;				// $t2 = j
innerFor:						#		for ( j=i; (j >= h) && (lst[j-h] > tmp); j = j-h) {
	blt $t2, $t0, innerForDone					# cmp j >= h
	move $t3, $t2								# $t3 = j
	sub $t4, $t3, $t0							# $t4 -= h
	mul $t4, $t4, 4								# $t4 * 4 (offset)
	add $t4, $t4, $a0							# $t4 += $a0 (starting addr)
	lw $t5, ($t4)								# $t5 = lst[j - h]
	ble $t5, $t9, innerForDone					# cmp lst[j-h] > tmp
	mul $t3, $t3, 4				#			lst[j] = lst[j-h];
	add $t3, $t3, $a0							# $t3 = addr lst[j]
	lw $t6, ($t3)								# $t6 = lst[j]
	sw $t5, ($t3)								# lst[j] = lst[j-h]
	sw $t6, ($t4)								# lst[j-h] = lst[j]
	sub $t2, $t2, $t0						# j = j-h
	j innerFor								# lp innerFor
innerForDone:					#		}
	move $t3, $t2				#		lst[j] = tmp;
	mul $t3, $t3, 4
	add $t3, $t3, $a0
	sw $t9, ($t3)
	add $t1, $t1, 1						# inc i
	j outerFor							# lp outerFor
outerForDone:					#	}
	div $t0, $t0, 3				#	h = h / 3;
	bnez $t0, secondWhile		# }
	jr $ra
...
findAverage:
	...
	move $s0, $a1				# $s0 = len
	jal findSum					# $v0 = sum
	mtc1 $v0, $f10			# move to coprocessor 1 $v0 -> $f10
	cvt.d.w $f10, $f10		# convert to double from word
	mtc1 $s0, $f8			# move to coprocessor 1 $s0 -> $f8
	cvt.d.w $f8, $f8		# convert to double from word
	div.d $f0, $f10, $f8	# div double $f0 = $f10(sum) / $f8(len)
	lw $ra, ($sp)			# pop $ra
	lw $s0, 4($sp)			# pop $s0
...
surfaceAreasStats:
	...
	lw $t0, ($a0)		# get min
	sw $t0, ($a2)		# store min
	sub $t0, $a1, 1		# $t0 = $a1(len) - 1
	mul $t1, $t0, 4		# $t1 = $t0 * 4 (byte offset)
	add $t2, $a0, $t1	# $t2 = $a0(starting addr) + $t1(offset)
	lw $t3, ($t2)		# get max
	sw $t3, ($a3)		# store max
	div $t1, $a1, 2		# $t1 = $a1(len) / 2
	mul $t1, $t1, 4		# $t1 = $t1 * 4 (byte offset)
	add $t1, $t1, $a0	# $t1 += $a0(starting addr)
	lw $t2, ($t1)
	sub $t1, $t1, 4
	lw $t3, ($t1)
	add $t0, $t2, $t3
	div $t0, $t0, 2		
	lw $t1, ($fp)		# get med addr
	sw $t0, ($t1)		# store med
	jal findAverage		# array already in $a0, len already in $a1
	lw $t0, 4($fp)		# get fave addr
	s.d $f0, ($t0)		# store(double) fave
